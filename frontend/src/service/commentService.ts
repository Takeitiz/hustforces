...
const commentService = {
    ...
    voteComment: async (id: string, isUpvote: boolean): Promise<CommentDto> => {
        try {
            const response = await apiClient.post<CommentDto>(`/comments/${id}/vote?upvote=${isUpvote}`)
            return response.data
        } catch (error) {
    ...
    getUserVote: async (commentId: string): Promise<string | null> => {
        try {
            const response = await apiClient.get<{ vote: string | null }>(`/comments/${commentId}/vote`)
